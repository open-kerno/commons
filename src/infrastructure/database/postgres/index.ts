...
const loadConfig = (configName: string): Partial<Config> => {
  const prefix = configName.toUpperCase();

  const envInt = (key: string) => {
    const val = process.env[`${prefix}_${key}`];
  ...
  return {
    ...
    port: envInt('PORT'),
    dbname: process.env[`${prefix}_DBNAME`],
    user: process.env[`${prefix}_USER`],
    password: process.env[`${prefix}_PASSWORD`],
    min: envInt('MIN'),
    max: envInt('MAX'),
    idle_timeout: envInt('IDLE_TIMEOUT'),
    connection_timeout: envInt('CONNECTION_TIMEOUT'),
    statement_timeout: envInt('STATEMENT_TIMEOUT'),
  };
};

const newPool = (configName: string): Pool => {
  const conf = loadConfig(configName);
