...
```ts
const enabled = svc.isEnabled('dark-mode', { userId: 'u-123' });
```

### `svc.getConfig<T>(key, context?, fallback?)`
...
```ts
import { FeatureContext, FeatureProvider } from '@open-kerno/commons/feature-management';

const myProvider: FeatureProvider = {
  initialize: async () => {
    await mySDK.connect();
  },
  isEnabled: (flag, context) => mySDK.isEnabled(flag, context?.userId),
  getConfig: <T>(key, context, fallback: T): T => {
...
| Scenario | `isEnabled` returns | `getConfig` returns |
|---|---|---|
| Provider init failed | `false` | `fallback` |
| Flag / key not found | `false` | `fallback` |
| Provider throws during evaluation | `false` | `fallback` |
| Provider not yet ready | `false` | `fallback` |
